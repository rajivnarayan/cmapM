function res = runCmapQuery(varargin)
% runCmapQuery run Cmap queries on a user specified dataset
% See runCMapQuery('--help') for details
...
if ~help_flag
    ...
    switch qo.query_mode
        ...
        case 'iterative'
            ...
            while ~isempty(ds_score) || ~isempty(ds_rank)
                ...
                [score(idx_start:idx_stop, :, :), leadf(idx_start:idx_stop, :, :)] = ...
                    mortar.compute.Connectivity.cmapScoreCore(qo.upind, qo.dnind,...
                    ds_rank, qo.max_rank,...
    ...
    res.cs = annotate_ds(res.cs, qo.col_meta, 'dim', 'row');
    if ~isempty(qo.query_meta)
        dbg(1, 'Anntating with custom query metadata');
        res.cs = annotate_ds(res.cs, qo.query_meta, 'dim', 'column');
    end
    % up score
...
% Load optinal annotations
if ~isempty(args.sig_meta)
    dbg(1, 'Using custom sig metadata');
    sig_meta = parse_record(args.sig_meta);
end
if ~isempty(args.query_meta)
    dbg(1, 'Using custom query metadata');
    res.query_meta = parse_record(args.query_meta);
end
...
if ~isempty(args.rank_score)
    ...
    if ~isempty(args.sig_meta)
        annot = annotate_ds(annot, sig_meta);
    end
    ...
    if isds(res.rank)
        ...
        if ~isempty(args.sig_meta)
            res.rank = annotate_ds(res.rank, sig_meta);
        end
        res.col_meta = gctmeta(res.rank);
        is_gctx = false;
...
if ~isempty(qo.col_meta)
    ds_rank = annotate_ds(ds_rank, qo.col_meta, 'dim', 'column');
end
end
