...
if args.save_digests
    ...
    saveResultsPerQuery(res, out_path, args.exemplar_field);
end

end

function saveResultsPerQuery(res, out_path, exemplar_field)
% save annotated GCT files for each query with ncs, cs and qvalues

% Build index table
index = gctmeta(res.ncs_result, 'column');
index = mvfield(index, 'cid', 'query_id');
nquery = length(index);
url = cell(nquery, 1);

query_id = res.ncs_result.cid;
ridx_exemplar = [];
if ~isempty(exemplar_field)
    ridx_exemplar = ds_get_meta(res.ncs_result, 'row', exemplar_field)>0;
end
...
for ii=1:nquery
    ...
    this_ds = ds_slice(res.ncs_result, 'cid', query_id(ii), 'ridx', ridx_exemplar, 'checkid', false);
    this_ds.cid = {'norm_cs'};
